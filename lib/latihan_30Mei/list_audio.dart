...
import 'package:audio_video_flutter_fauzan_2201092046/Latihan_30Mei/model_audio.dart';
...
class _PageLatAudioState extends State<PageLatAudio> {
  ...
  Future<void> _fetchAudioData() async {
    try {
      final response = await http.get(Uri.parse('http://192.168.158.51/latAudio/getAudio.php'));
  ...
  void _play(int index) async {
    final audioUrl = 'http://192.168.158.51/latAudio/audio/${_filteredAudioList[index].audio}';
                ...
                return Card(
                  ...
                  child: ListTile(
                    leading: Image.network(
                      'http://192.168.158.51/latAudio/gambar/${_filteredAudioList[index].gambar}',
                      width: 50,
                      height: 50,
                      fit: BoxFit.cover,
